...
def create_all_data(symbol, total_page=3):
...
def writeCSVFile(df, symbol, dttm_fmt, output_path=DIR_SEC_CSV, include_index=False):

    file_name = dttm_fmt + "_" + symbol
    print(file_name)
...
if __name__ == "__main__":
    ...
    for symbol in symbol_list:
        ...
        dttm_fmt = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        writeCSVFile(df, symbol, dttm_fmt)