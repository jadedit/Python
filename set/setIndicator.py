...
def rsi(df):
    ...
    df_avg_gain.ix[periods] = df_gain[
                              1:periods + 1].mean()  # First Average Gain = Sum of Gains over the past 14 periods / 14.
    df_avg_loss.ix[periods] = df_loss[
                              1:periods + 1].mean()  # First Average Loss = Sum of Losses over the past 14 periods / 14
...
def isStrongSR(df_sr, window=10):
    ...
    return 1 * (
    df_sum == window)  # True When Sharpe ratio > 10 continually (by default), multiply 1 to convert integer number
